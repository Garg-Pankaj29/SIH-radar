  ...
  return (
    ...
    >
      ...
      <div className="card" style={{ marginBottom: "28px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", fontWeight: "700", fontSize: "0.85rem", marginBottom: "14px", textTransform: "uppercase", color: "var(--text-secondary)" }}>
          <LuUsers size={16} color="var(--accent-light)" />
          <span>Select Team Core Skills ({teamSkills.length} selected):</span>
        </div>
        <div className="skill-grid">
          {SKILLS.map((skill) => {
            ...
            return (
              ...
              >
                {isSelected ? <LuCheck size={13} /> : <LuPlus size={13} />}
                <span>{skill}</span>
              </button>
            );
          })}
          ...
          <div key={p.ps_number} className="card" style={{ padding: "16px 20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "12px" }}>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>Skill Alignment Score</div>
              </div>
            </div>
